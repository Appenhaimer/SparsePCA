...
log_dir = "A:\\iTech\\CS282\\Project\\out\\anytimepca_results" # PATCHED_FOR_CUSTOM_OUTPUT_DIR
os.makedirs(log_dir, exist_ok=True)
log_file = os.path.join(log_dir, 'sspca_log.txt')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fpath", help="relative path to numeric data including filename", type=str)
    ...
    explained_variance = se_mp_var_star
    explained_ratio = se_mp_var_star / max_trace if max_trace > 0 else 0

    print(f"Explained Variance (trace): {explained_variance:.6f}")
    print(f"Explained Variance Ratio: {explained_ratio:.6f}")
    ...
    # 创建以日期命名的目录
    date_dir = strftime("%y_%m_%d")
    if(date_dir not in os.listdir('out/') ):
        os.mkdir( 'out/{}'.format(date_dir)  )
    ...
    output_filename = 'sspcaExp_{}_kstar{}_k{}_{}.csv'.format(dataset_name, k_star, k, timestamp)
    
    # 保存结果
    output_path = os.path.join('out', date_dir, output_filename)
    toSave.to_csv(output_path, index=False)
    ...
    # 清理临时文件
    if os.path.exists('sspca_tmp_{}_{}.csv'.format(k, k_star)):
        os.remove('sspca_tmp_{}_{}.csv'.format(k, k_star))
    if os.path.exists('sspca_tmp_sigma_{}_{}.csv'.format(k, k_star)):
